...
const Product = ({
  ...
  const { addItemDataToCart, cartItems, toggleCart, tfLabel, removeItemDataToCart } =
    useContext(DailyItemsList);
  const inCart = cartItems.some(item => item.id === item_id);
  ...
  return (
    <div
      className={`product border-0 sm:w-[13%] md:w-[30%] xl:w-[13%]  w-[100%] sm:overflow-hidden  sm:shrink-0 h-[300px] p-5 flex flex-col justify-center gap-2 p-0.2 shadow-[0_2px_5px_-1px_rgba(50,50,93,0.25),0_1px_3px_-1px_rgba(0,0,0,0.3)]  m-2 ${toggleCart ? "pointer-events-none" : ""
        } `}
    >
      <div className="product-image w-[80%] m-auto">
        {item_image ? (
          <img
            ...
            alt={item_name}
            height="400"
            className={`rounded-2xl ${toggleCart ? "opacity-40" : ""
              } h-32 mx-auto`}
          />
        ) : (
          <ShimmerThumbnail />
      ...
      <div className="quantity">
        <p className="text-xs">{item_quantity || "NA"}{item_quantity_unit}</p>
      </div>
